...
(function (global) {
  ...
  var RULES = [
    ...
    { re: /^LBL\[\s*(\d+)\s*(?::([^\]]*))?\]/, fn: function (m) { return 'Label ' + m[1] + label(m[2]) + ' — a jump target; does nothing by itself.'; } },
    { re: /^JMP\s+LBL\[\s*(\d+)\s*(?::([^\]]*))?\]/, fn: function (m) { return 'Jump unconditionally to label ' + m[1] + label(m[2]) + '.'; } },
